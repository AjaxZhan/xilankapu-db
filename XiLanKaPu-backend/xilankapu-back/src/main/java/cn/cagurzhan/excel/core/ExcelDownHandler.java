package cn.cagurzhan.excel.core;

import cn.cagurzhan.excel.annotation.ExcelDictFormat;
import cn.cagurzhan.excel.annotation.ExcelEnumFormat;
        ...
        for (Map.Entry<Integer, FieldWrapper> entry : fieldCache.getSortedFieldMap().entrySet()) {
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (StrUtil.isNotBlank(dictType)) {
                    // 如果传递了字典名，则依据字典建立下拉
                    // TODO 这里为了方便，将dictService.getAllDictByDictType(dictType)改成普通hashmap，因为我不会用到这个功能
                    // TODO 如果需要这个功能，请务必重新注释回去！
                    Collection<String> values = Optional.ofNullable(new HashMap<String,String>())
